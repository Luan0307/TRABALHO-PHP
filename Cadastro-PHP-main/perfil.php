...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrfToken($_POST['_csrf'] ?? '')) {
        ...
        if ($cpf === '' || $endereco === '' || $numero === '' || $bairro === '' || $cidade === '' || $estado === '' || $cep === '' || $telefone === '') {
            $erro = 'Preencha todos os campos obrigatórios para finalizar pedidos.';
        } elseif (!preg_match('/^[A-Z]{2}$/', $estado)) {
            $erro = 'Informe a UF com 2 letras (ex: DF, SP, RJ).';
        } elseif (strlen($cpfLimpo) !== 11) {
            $erro = 'Informe um CPF válido com 11 números.';
        } else {
            $update = $pdo->prepare('
                UPDATE usuarios
                SET cpf = :cpf,
        ...
        <div class="card-body">
            ...
            <form method="post" class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" id="cep" name="cep" class="form-control" value="<?= old('cep', $usuario['cep'] ?? '') ?>" required>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" id="endereco" name="endereco" class="form-control" value="<?= old('endereco', $usuario['endereco'] ?? '') ?>" required>
                </div>
                <div class="col-md-2">
                    <label for="numero" class="form-label">Numero</label>
                    <input type="text" id="numero" name="numero" class="form-control" value="<?= old('numero', $usuario['numero'] ?? '') ?>" required>
                </div>
                <div class="col-md-4">
                    <label for="bairro" class="form-label">Bairro</label>
                    <input type="text" id="bairro" name="bairro" class="form-control" value="<?= old('bairro', $usuario['bairro'] ?? '') ?>" required>
                </div>

                <div class="col-md-8">
                    <label for="cidade" class="form-label">Cidade</label>
                    <input type="text" id="cidade" name="cidade" class="form-control" value="<?= old('cidade', $usuario['cidade'] ?? '') ?>" required>
                </div>
                <div class="col-md-4">
                    <label for="estado" class="form-label">UF</label>
                    <input type="text" id="estado" name="estado" maxlength="2" class="form-control" value="<?= old('estado', $usuario['estado'] ?? '') ?>" required>
                </div>
